...
const WEB3FORMS_KEY = process.env.WEB3FORMS_BOOKING_KEY || '874d9b71-c185-4a8c-96a0-aba446ac3bf7';
...
// bots, not a real applicant. They still land on the dashboard. Set to 0 to
// email literally every landing.
...
async function ingest(req, res) {
  ...
  // Stage only moves forward — a late 'landed' beacon can't undo 'booked'.
...
async function maybeEmail(redis, j) {
  ...
  const idLabel = j.fullName || j.email || 'Anonymous visitor';
  ...
  if (j.email) fields.Email = j.email;
  if (j.phone) fields.Phone = j.phone;
